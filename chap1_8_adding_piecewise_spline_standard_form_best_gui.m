...
function addUI()
    ...
    [xSliderMin xSliderMax] = getMinMaxX(1);
    yMin = -10;
    yMax = 10;

    xSlider = uicontrol('style','slider','units','pixel','position',[SLIDER_POS_X 20 300 20],...
        'sliderstep',[XY_SLIDER_STEP/(xSliderMax-xSliderMin), XY_SLIDER_STEP * 5/(xSliderMax-xSliderMin)],'max',xSliderMax,'min',xSliderMin, 'value',global_splines_data{1}(1,1));
    addlistener(xSlider,'ContinuousValueChange',@(hObject, event) sliderPlot_x(hObject, event));
    ...
    ySlider = uicontrol('style','slider','units','pixel','position',[SLIDER_POS_X 0 300 20],...
        'sliderstep',[XY_SLIDER_STEP/(yMax-yMin), XY_SLIDER_STEP * 5/(yMax-yMin)],'max',yMax,'min',yMin, 'value',global_splines_data{2}(1,1));
    global_splines_data{5} = ySlider; % required so that menu selection can update the curve point impacted by the y slider 
    addlistener(ySlider,'ContinuousValueChange',@(hObject, event) sliderPlot_y(hObject, event));
    uicontrol('style','text',...
...
function [minX maxX] = getMinMaxX(pointIndex)
    ...
    if pointIndex == 1
        ...
        prevPointX = global_splines_data{1}(1, pointIndex - 1) + XY_SLIDER_STEP;
    ...
    if pointIndex == 8
        ...
        nextPointX = global_splines_data{1}(1, pointIndex + 1) - XY_SLIDER_STEP;
...
function sliderPlot_x(hObject,event)
    global global_splines_data;
...
function sliderPlot_y(hObject,event)
    global global_splines_data;
    n = get(hObject,'Value');
...
function updateSliderXProperties(slider, sliderTextValue, pointIndex)
    global XY_SLIDER_STEP;
    global global_splines_data;

    [xSliderMin xSliderMax] = getMinMaxX(pointIndex);
    slider.Max = xSliderMax;
    slider.Min = xSliderMin;
    slider.SliderStep = [XY_SLIDER_STEP/(xSliderMax-xSliderMin), XY_SLIDER_STEP * 5/(xSliderMax-xSliderMin)];
...
function menuSelection(hObject,event)
    ...
    switch global_splines_data{3}
        case 'P1'
            updateSliderXProperties(xSlider, xSliderTextValue, 1)
            
            yValue = global_splines_data{2}(1,1);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P2'
            updateSliderXProperties(xSlider, xSliderTextValue, 2)
            
            yValue = global_splines_data{2}(1,2);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P3'
            updateSliderXProperties(xSlider, xSliderTextValue, 3)
            
            yValue = global_splines_data{2}(1,3);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P4'
            updateSliderXProperties(xSlider, xSliderTextValue, 4)
            
            yValue = global_splines_data{2}(1,4);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P5'
            updateSliderXProperties(xSlider, xSliderTextValue, 5)
            
            yValue = global_splines_data{2}(1,5);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P6'
            updateSliderXProperties(xSlider, xSliderTextValue, 6)
            
            yValue = global_splines_data{2}(1,6);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P7'
            updateSliderXProperties(xSlider, xSliderTextValue, 7)
            
            yValue = global_splines_data{2}(1,7);
            ySlider.Value = yValue;
            ySliderTextValue.String = yValue;
        case 'P8'
            updateSliderXProperties(xSlider, xSliderTextValue, 8)
